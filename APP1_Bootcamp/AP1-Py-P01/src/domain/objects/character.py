...
class Character:
    ...
    def _load(self, **kwargs):
        for key, value in kwargs.items():
            match key:
                ...
                case "effects":
                    for eff in value:
                        self.__effects[Effects[eff["effect"]]].append(RogueEffect(Effects[eff["effect"]], eff["value"], eff["period"]))
                case "items":
                    for dct in value:
    ...
    def __str__(self) -> str: return f"Ник: {self.nickname}, сила: {self.strength}, ловкость: {self.agility}, хп {self.__hp}/{self.__max_hp}"
