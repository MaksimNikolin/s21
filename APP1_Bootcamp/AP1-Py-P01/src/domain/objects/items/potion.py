...
class Potion(Item):
    ...
    def pick_up(self) -> list[RogueEvent]: return [RogueEvent(f"You find a potion: Potion {self.feature.label} {self.power}", 11)]

    def drop(self) -> tuple[list[RogueEvent], Item]: return [RogueEvent(f"You dropped a potion: Potion {self.feature.label} {self.power}")], self

    def use(self) -> tuple[list[RogueEvent], RogueEffect]:
        RogueStats.get_instance().used_potions += 1
        # if self.feature == Effects.AGILITY: spelling = "Agility temporary increase"
        # elif self.feature == Effects.STRENGTH: spelling = "Strength temporary increase"
        # else: spelling = "Health temporary increase"
    ...
    def __str__(self): return f"Potion {self.feature.label} {self.power}"
