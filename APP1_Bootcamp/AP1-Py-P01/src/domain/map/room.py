...
from domain.map.settings import (ENEMY_CREATION_PROB, ITEM_CREATION_PROB, ROOM_INDENT, ROOM_MINIMUM_HEIGHT, ROOM_MINIMUM_WIDTH)
...
class Room:
    room_symbol, room_symbol_color, room_border_color = ".", 2, 5
    ...
    @property
    def visited(self): return self.__visited

    @property
    def doors(self) -> list[Door]: return self.__doors
    ...
    def get_cell(self, y: int, x: int) -> tuple[str, int]:
        if not self.__visited and self.has_character: self.__visited = True
        crd = (y, x)
        if crd in self.__objects and self.__objects[crd].is_visible:
            return self.__objects[crd].symbol, self.__objects[crd].color
        if crd in self.__exit: return self.__exit[crd].symbol, self.__exit[crd].color
        if crd in self.__items: return self.__items[crd].symbol, self.__items[crd].color
        if crd in self.__keys: return self.__keys[crd].symbol, self.__keys[crd].color
        return self.room_symbol, self.room_symbol_color

    def get_border_symbol(self, y, x) -> tuple[str, int]:
        if (y, x) in self.__doors_coordinates: return " ", Corridor.color

        char = ""
        if (x == self.x - 1 or x == self.x_ + 1) and self.y <= y <= self.y_: char = "║"
        elif (y == self.y - 1 or y == self.y_ + 1) and self.x <= x <= self.x_: char = "═"
        elif y == self.y - 1 and x == self.x - 1: char = "╔"
        elif x == self.x - 1 and y == self.y_ + 1: char = "╚"
        elif x == self.x_ + 1 and y == self.y - 1: char = "╗"
        elif x == self.x_ + 1 and y == self.y_ + 1: char = "╝"
    ...
    def generate_keys(self):
        for key in self.has_keys: self.__keys[self.__allocate_coordinates()] = Key(key)

    def get_random_crd_in_zone(self, crd: Coordinate, radius: int) -> Coordinate:
        if not radius: return self.__allocate_coordinates()
        y, x = crd
    ...
    def __remove_side(self, side: str):
        if side in self.__sides: self.__sides.remove(side)
    ...
    def add_object(self, crd: Coordinate, obj: Any): self.__objects[crd] = obj

    def remove_object(self, crd: Coordinate): del self.__objects[crd]

    def get_object(self, crd: Coordinate) -> Any | None: return self.__objects.get(crd)

    @property
    def objects(self): return self.__objects.copy()

    def get_exit(self, crd: Coordinate) -> Any | None: return self.__exit.get(crd)
    ...
    def face_door(self, crd: Coordinate):
        if crd in self.__doors_coordinates:
            ...
            if door and door[0].is_closed: return door[0]
        return None

    def add_item(self, crd: Coordinate, obj: Any): self.__items[crd] = obj

    def remove_item(self, crd: Coordinate): del self.__items[crd]

    def get_item(self, crd: Coordinate) -> Any | None: return self.__items.get(crd, None)

    def is_exit(self, crd: Coordinate) -> bool: return crd in self.__exit
