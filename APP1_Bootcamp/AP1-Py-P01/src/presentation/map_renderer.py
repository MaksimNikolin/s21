...
class MapRenderer:
    ...
    def show_stats(self, stats: list[dict]):
        ...
        def __render(slot_: EventSlot, index: int):
            ...
            render_string = (
                f"{str(stats_data['Никнейм'])[:12].center(12)} | "
                f"{str(stats_data['Золото']).center(6)} | "
                f"{str(stats_data['Уровень подземелья']).center(10)} | "
                f"{str(stats_data['Поверженные враги']).center(11)} | "
                f"{str(stats_data['Съеденной еды']).center(9)} | "
                f"{str(stats_data['Использованных зелий']).center(5)} | "
                f"{str(stats_data['Использованных свитков']).center(5)} | "
                f"{str(stats_data['Нанесенных ударов']).center(10)} | "
                f"{str(stats_data['Промахов']).center(8)} | "
                f"{str(stats_data['Пройденных клеток']).center(10)}"
            )
