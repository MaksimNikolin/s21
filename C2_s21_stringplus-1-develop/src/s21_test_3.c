...
START_TEST(test_for_strtok) {
  ...
  ck_assert(token2 == s21_NULL);

  char test3[] = ",,,";
  char *token3 = s21_strtok(test3, " ,!");
  ck_assert(token3 == s21_NULL);
...
START_TEST(test_for_to_lower) {
  ...
  if (new_str2 != s21_NULL) {
    ck_assert_str_eq(new_str2, "world!");
    free(new_str2);
  }

  char *new_str3 = s21_to_lower(str3);
  if (new_str3 != s21_NULL) {
    ck_assert_str_eq(new_str3, "hello");
    free(new_str3);
  }
}
END_TEST
...
START_TEST(test_for_to_upper) {
  ...
  char *new_str3 = s21_to_upper(str3);
  if (new_str3 != s21_NULL) {
    ck_assert_str_eq(new_str3, "HELLO?~");
    free(new_str3);
  }
