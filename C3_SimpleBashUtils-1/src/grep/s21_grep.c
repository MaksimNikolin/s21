...
int parser(int argc, char *argv[], Flags *flag, Grep_values *value, char patterns[SIZE][SIZE]) {
  int opt = 0;
...
void grep(Grep_values value, Flags flag, char pattern[SIZE][SIZE]) {
  ...
  if (access(value.path, F_OK) == 0) {
    ...
    FILE *filename;
    int count_lines = 0, count_match_lines = 0, comp_flag = REG_EXTENDED, stop = 0;
    char string[SIZE] = " ";
